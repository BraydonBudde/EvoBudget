...
function defaultState() {
  ...
  return {
    settings: { currency:'USD', symbol:'$', periodStart:start, periodEnd:end, language:'en', automationEnabled:true },
    rollover: 0,
    budgets: {
...
function runDebtPayoff() {
  ...
  while (working.some(d=>d.remaining>0.01)&&month<600) {
    ...
    for (const d of working) {
      ...
      const pay=Math.min(d.remaining,d.minimumPayment);
      d.remaining-=pay;
      if (d.remaining<0.01){freed+=d.minimumPayment-Math.max(0,d.minimumPayment-pay);d.remaining=0;if(!d.paidOffMonth)d.paidOffMonth=month;}
    }
...
const TRANSLATIONS = {
  en: {
    ...
    dpc_th_name:'Debt name',dpc_th_type:'Type',dpc_th_balance:'Balance',
  ...
  de: {
    ...
    dpc_th_name:'Schuldenname',dpc_th_type:'Art',dpc_th_balance:'Saldo',
    ...
    dpc_th_name:'Nom de la dette',dpc_th_type:'Type',dpc_th_balance:'Solde',
    ...
    dpc_th_name:'Nombre de la deuda',dpc_th_type:'Tipo',dpc_th_balance:'Saldo',
  ...
  it: {
    ...
    dpc_th_name:'Nome del debito',dpc_th_type:'Tipo',dpc_th_balance:'Saldo',
  ...
  pl: {
    ...
    dpc_th_name:'Nazwa długu',dpc_th_type:'Typ',dpc_th_balance:'Saldo',
...
function renderDashboard() {
  ...
  const expDebt=state.debts.reduce((s,d)=>s+(d.minimumPayment||0),0);
...
function openDebtModal(debtId){
  ...
  document.getElementById('modalBody').innerHTML=`
    ...
    <div class="field-grid">
      ...
      <div class="field"><label class="field-label">${t('dpc_th_apr')} (%)</label><input class="input" type="number" id="debtApr" min="0" max="100" step="0.01" placeholder="0.00" value="${d?.interestRate||''}"></div>
    </div>
    <div class="field-grid">
      <div class="field"><label class="field-label">${t('dpc_th_min')} (${SYM}) <span class="required-star" aria-hidden="true">*</span></label><input class="input" type="number" id="debtMin" min="0" step="0.01" placeholder="0.00" value="${d?.minimumPayment||''}"></div>
  ...
  ['debtName','debtBalance','debtMin','debtDueDay'].forEach(id=>document.getElementById(id)?.addEventListener('input',()=>{document.getElementById(id)?.classList.remove('fk-invalid');clearDebtErr();}));
  ...
  document.getElementById('saveDebtBtn')?.addEventListener('click',()=>{
    ...
    const name=nameEl?.value.trim(),type=document.getElementById('debtType')?.value,balance=parseFloat(balEl?.value)||0,apr=parseFloat(document.getElementById('debtApr')?.value)||0,minPay=parseFloat(minEl?.value)||0,dueDay=parseInt(dueEl?.value,10);
    const auto=document.getElementById('automateToggle')?.checked;
    [nameEl,balEl,minEl,dueEl].forEach(x=>x&&x.classList.remove('fk-invalid'));
    let bad=false;
    if(!name){nameEl?.classList.add('fk-invalid');bad=true;}
    if(!(balance>0)){balEl?.classList.add('fk-invalid');bad=true;}
    if(!(minPay>0)){minEl?.classList.add('fk-invalid');bad=true;}
    ...
    if(errEl)errEl.hidden=true;
    let did,debtObj;
    if(isNew){did=uid();debtObj={id:did,name,type,balance,interestRate:apr,minimumPayment:minPay,dueDay:isNaN(dueDay)?'':dueDay};state.debts.push(debtObj);}
    else{did=debtId;debtObj=state.debts.find(x=>x.id===debtId);if(debtObj){debtObj.name=name;debtObj.type=type;debtObj.balance=balance;debtObj.interestRate=apr;debtObj.minimumPayment=minPay;debtObj.dueDay=isNaN(dueDay)?'':dueDay;}}
    if(automationOn()&&debtObj){if(auto)upsertLinkedTemplate('debt',did,{type:'debt',category:name||'Debt',label:(name||'Debt')+' '+t('automate_payment_word'),amount:minPay,frequency:'monthly',nextDue:nextDueFromDay(dueDay)});else removeLinkedTemplate('debt',did);}
...
function renderDebt(){
  ...
  el.innerHTML=`<div class="section-header"><h2 class="section-title">💳 ${t('dpc_title')}</h2><div class="section-header-actions">${helpBtn('debt')}<button class="btn btn-ghost btn-sm" id="addDebtBtn">${t('dpc_add_btn')}</button></div></div>
    ...
    ${state.debts.length>0&&result?`<div class="debt-results">
      <div class="debt-results-cards">
        ...
        <div class="dr-card dr-card--blue"><div class="dr-label">${t('dpc_monthly_label')}</div><div class="dr-value">${fmt(totMin+(extraPayment||0))}</div><div class="dr-sub">${fmt(totMin)} ${t('dpc_min_abbr')} + ${fmt(extraPayment||0)} ${t('dpc_extra_abbr')}</div></div>
      </div>
      <div class="panel" style="margin-top:16px"><div class="panel-inner-sm">
        ...
        ${result.payoffOrder.map((d,i)=>`<div class="payoff-order-row"><span class="po-num">${i+1}</span><div class="po-info"><div class="po-name">${esc(d.name)}</div><div class="po-detail">${DT[d.type]||d.type} • ${fmt(d.balance)} ${t('dpc_balance_word')} • ${d.interestRate}% ${t('dpc_apr_word')}</div></div><div class="po-date">${t('dpc_paid_off')} <strong>${d.paidOffDate}</strong></div></div>`).join('')}
      </div></div>
    </div>`:''}`;
  ...
  el.querySelectorAll('.debt-auto-cb[data-debt-auto]').forEach(cb=>cb.addEventListener('change',()=>{
    ...
    if(cb.checked){
      ...
      upsertLinkedTemplate('debt',d.id,{type:'debt',category:d.name||'Debt',label:(d.name||'Debt')+' '+t('automate_payment_word'),amount:d.minimumPayment,frequency:'monthly',nextDue:nextDueFromDay(d.dueDay)});
      showToast(t('recurring_saved'));
...
function renderSubscriptions(){
  ...
  const catSegs=catEntries.map(([label,value],i)=>({label,value,color:COLORS[i%COLORS.length],pct:totMo>0?value/totMo*100:0}));

  // Build summary bar HTML
  const summaryBar = state.subscriptions.length > 0
    ? `<div class="sub-summary-bar">
        <div class="sub-sum-item"><div class="sub-sum-label">Monthly total</div><div class="sub-sum-value">${fmt(totMo)}</div></div>
        <div class="sub-sum-item"><div class="sub-sum-label">Annual total</div><div class="sub-sum-value">${fmt(totYr)}</div></div>
        <div class="sub-sum-item"><div class="sub-sum-label">Active</div><div class="sub-sum-value">${active.length}</div></div>
        <div class="sub-sum-item"><div class="sub-sum-label">Paused</div><div class="sub-sum-value">${state.subscriptions.length-active.length}</div></div>
      </div>` : '';
  ...
  const listHtml = state.subscriptions.length === 0
    ? `<div class="sub-empty-centered">
        ...
        <p style="font-size:15px;font-weight:600;color:var(--text-primary);margin:0">No subscriptions yet.</p>
        <p class="empty-sub" style="margin:4px 0 0">Add your recurring payments - Netflix, Spotify, gym memberships, etc.</p>
      </div>`
    : state.subscriptions.map(sub => {
        const freqLabel = sub.frequency==='annual'?'year':sub.frequency==='quarterly'?'quarter':'month';
        const nextDue = sub.nextBillingDate ? `<span class="sub-due">Next: ${formatDateDisplay(sub.nextBillingDate)}</span>` : '';
          ...
          <div class="sub-card-inner">
            <div class="sub-card-head">
              <div class="sub-card-left">
                ...
                <div class="sub-meta"><span class="sub-cat-pill">${esc(sub.category||'Other')}</span>${nextDue}</div>
              </div>
            ...
            <div class="sub-card-foot">
              ...
              <div class="sub-btn-row">
                <button class="sf-edit-btn btn-icon-tiny" data-sub-edit="${sub.id}" type="button" title="Edit">✏️</button>
                <button class="del-btn" data-sub-del="${sub.id}" type="button" title="Delete">×</button>
              </div>
            </div>
          </div>
        </div>`;
  ...
  const chartHtml = catEntries.length > 0
    ? `<div class="panel">
        <div class="panel-inner-sm">
          <div class="panel-title-sm" style="margin-bottom:14px">By category</div>
            ...
            <div class="donut-legend">
              ${catSegs.map(s=>`<div class="dleg-row">
                ...
                <div class="dleg-stack">
                  ...
                  <span class="dleg-sub-amt">${fmt(s.value)}/month</span>
                </div>
              </div>`).join('')}
            </div>
          </div>
  ...
  el.innerHTML = `
    <div class="section-header">
      ...
      <div class="section-header-actions">
        ...
        <button class="btn btn-ghost btn-sm" id="addSubBtn">+ Add subscription</button>
      </div>
    </div>
    <p class="section-desc">Track every recurring payment and understand your true annual cost. Pause subscriptions you're not using to keep costs in check.</p>
    ${summaryBar}`;
...
function openSubModal(subId){
  ...
  const allocEnabled=state.allocation?.enabled;
  document.getElementById('modalTitle').textContent=isNew?'🔄 Add Subscription':'✏️ Edit Subscription';
  document.getElementById('modalBody').innerHTML=`<div class="field"><label class="field-label">Subscription name <span class="required-star" aria-hidden="true">*</span></label><input class="input" type="text" id="subName" placeholder="e.g. Netflix" value="${esc(sub?.name||'')}"></div>
    <div class="field-grid"><div class="field"><label class="field-label">Amount (${SYM}) <span class="required-star" aria-hidden="true">*</span></label><input class="input" type="number" id="subAmount" min="0" step="0.01" placeholder="0.00" value="${sub?.amount||''}"></div><div class="field"><label class="field-label">Billing frequency</label><select class="select" id="subFreq"><option value="monthly" ${sub?.frequency==='monthly'?'selected':''}>Monthly</option><option value="annual" ${sub?.frequency==='annual'?'selected':''}>Annual</option><option value="quarterly" ${sub?.frequency==='quarterly'?'selected':''}>Quarterly</option><option value="weekly" ${sub?.frequency==='weekly'?'selected':''}>Weekly</option></select></div></div>
    <div class="field"><label class="field-label">Category</label><select class="select" id="subCat">${SUB_CATS.map(c=>`<option value="${c}" ${sub?.category===c?'selected':''}>${c}</option>`).join('')}</select></div>
    ...
    <div class="field"><label class="field-label">Next billing date <span class="required-star" aria-hidden="true">*</span></label>${styledDateField('subDate','subDateWrap',sub?.nextBillingDate||'')}</div>
    ${automateRow(!!findLinkedTemplate('subscription',subId))}
    <div class="tx-error" id="subError" hidden></div>
    <div class="edit-tx-actions"><button class="btn btn-primary" id="saveSubBtn">${isNew?'Add subscription':'Save changes'}</button><button class="btn btn-ghost btn-sm" id="cancelSubBtn">Cancel</button>${!isNew?`<button class="btn btn-danger btn-sm" id="deleteSubBtn">Delete</button>`:''}</div>`;
  document.getElementById('tutorialOverlay').hidden=false;
...
<div style="margin-top:18px;text-align:center"><a href="https://www.youtube.com/results?search_query=personal+budget+planner+tutorial" target="_blank" rel="noopener noreferrer" class="help-video-btn"><svg width="16" height="16" viewBox="0 0 24 24"><path fill="white" d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.77 0 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.34 6.34 0 0 0-6.33 6.34 6.34 6.34 0 0 0 12.67 0l-.01-11.7A8.27 8.27 0 0 0 21 5.11V2a4.84 4.84 0 0 1-1.41 4.69z"/></svg> ${t('video_tutorial')}</a></div>`
  ...
  subscriptions:{title:'🔄 Subscription Tracker',body:`
<p>Track every recurring payment and understand your true monthly and annual cost. Subscriptions that quietly drain your account are easy to miss - this keeps them visible.</p>
<h4 style="margin:14px 0 6px;font-size:14px">Adding a subscription</h4>
<ol style="margin:0;padding-left:18px;line-height:2">
  <li>Click <strong>+ Add subscription</strong></li>
  <li>Enter the <strong>name, amount, and billing frequency</strong> (monthly, annual, quarterly, weekly)</li>
  <li>Pick a <strong>category</strong> to group similar subscriptions</li>
  <li>Set the <strong>next billing date</strong> - it will appear on the Smart Calendar</li>
</ol>
<h4 style="margin:14px 0 6px;font-size:14px">Monthly equivalent</h4>
<p>Annual and quarterly subscriptions are converted to a monthly cost so you can see your true monthly spend at a glance.</p>
<h4 style="margin:14px 0 6px;font-size:14px">Pausing subscriptions</h4>
<p>Hit <strong>⏸ Pause</strong> on any subscription you're not currently using. It won't count toward your totals until you resume it.</p>
<h4 style="margin:14px 0 6px;font-size:14px">Category chart</h4>
<p>The donut chart shows how your subscription spending breaks down by category - hover a segment to see the details.</p>
